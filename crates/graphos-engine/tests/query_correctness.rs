...
//! ```bash
//! cargo test -p graphos-engine --features full --test query_correctness
//! ```
//!
//! Run for specific query language:
//! ```bash
//! cargo test -p graphos-engine --features gql --test query_correctness -- gql
//! cargo test -p graphos-engine --features cypher --test query_correctness -- cypher
//! cargo test -p graphos-engine --features gremlin --test query_correctness -- gremlin
//! cargo test -p graphos-engine --features graphql --test query_correctness -- graphql
//! ```
...
fn create_social_network() -> GraphosDB {
...
fn create_numeric_data() -> GraphosDB {
    let db = GraphosDB::new_in_memory();
    let session = db.session();
